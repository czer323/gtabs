...
Each spawn is one review cycle. Re-spawned after each fix iteration.

## Workflow

```mermaid
flowchart TD
    Start(["Spawned by implementer"]) --> Context[Understand context]
    Context --> A1[Read acceptance criteria and spec]
...
<procedure>
1. Fetch diff: `git diff`, `gh pr diff`, or github tool
