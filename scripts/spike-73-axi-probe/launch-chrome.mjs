#!/usr/bin/env node
/**
 * Card #73 spike probe — launcher.
 *
 * Starts a LIVE Chrome session running the unpacked gTabs extension (dist/) on
 * a remote-debugging port so chrome-devtools-axi can attach to it via
 * CHROME_DEVTOOLS_AXI_BROWSER_URL. Keeps running until killed (Ctrl-C).
 *
 * Usage:
 *   node scripts/spike-73-axi-probe/launch-chrome.mjs [--port 9222]
 *
