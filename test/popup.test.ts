...
describe("Popup Page", () => {
  beforeEach(() => {
    ...
    vi.spyOn(storage, "getSuggestions").mockResolvedValue([
      { name: "Pending Group", color: "blue", tabs: [{ url: "tab1.com" }] },
    ] as any);
  ...
  it("initializes and binds core buttons", async () => {
    ...
    (chrome.runtime.sendMessage as any).mockImplementationOnce((msg: any, cb: Function) => {
      if (msg.type === "organize")
        cb({
          suggestions: [
            { name: "Group 1", color: "blue", tabs: [{ url: "a.com" }, { url: "b.com" }] },
          ],
        });
    });
    btnOrganize.click();
    ...
    (chrome.runtime.sendMessage as any).mockImplementationOnce((_msg: any, cb: Function) =>
      cb(undefined),
    );
    btnOrganize.click();
    for (let i = 0; i < 10; i++) await new Promise((r) => process.nextTick(r));
