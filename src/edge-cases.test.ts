...
describe("hostnameFromUrl – edge cases", () => {
  ...
  it("strips only leading www", () =>
    expect(hostnameFromUrl("https://wwwexample.com")).toBe("wwwexample.com"));
  it("handles IPv4 addresses", () =>
    expect(hostnameFromUrl("http://192.168.1.1/admin")).toBe("192.168.1.1"));
  it("handles URL with auth info", () =>
    expect(hostnameFromUrl("https://user:pass@example.com/path")).toBe("example.com"));
  it("handles deep subdomains", () =>
    expect(hostnameFromUrl("https://a.b.c.d.example.com")).toBe("a.b.c.d.example.com"));
  it("handles www.www prefix", () =>
    expect(hostnameFromUrl("https://www.www.example.com")).toBe("www.example.com"));
  it('returns "newtab" for chrome://newtab (URL parses the "host")', () =>
    expect(hostnameFromUrl("chrome://newtab")).toBe("newtab"));
  it("handles URL with fragment", () =>
    expect(hostnameFromUrl("https://example.com/page#section")).toBe("example.com"));
  it("handles URL with query string", () =>
    expect(hostnameFromUrl("https://example.com/search?q=test")).toBe("example.com"));
  it("handles port in URL correctly", () =>
    expect(hostnameFromUrl("https://api.example.com:8443/v1")).toBe("api.example.com"));
});
...
describe("workspace storage – edge cases", () => {
  ...
  it("saveWorkspace overwrites existing workspace with same name", async () => {
    ...
    await saveWorkspace("ws1", {
      name: "ws1",
      savedAt: 2,
      tabs: [{ url: "https://x.com", title: "X", pinned: false, active: false }],
    });
    const ws = await getWorkspaces();
...
describe("saveCurrentWorkspace", () => {
  ...
  it("throws when window id is undefined", async () => {
    ...
    await expect(saveCurrentWorkspace("fail")).rejects.toThrow(
      "Could not determine current window",
    );
  });
});
