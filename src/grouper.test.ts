...
describe("applyDomainRules", () => {
  ...
  it("does not treat exact domains as prefix or subdomain matches", () => {
    const subdomainTabs: TabInfo[] = [
      { id: 22, title: "API", url: "https://api.github.com/repos" },
    ];
    const rules: DomainRule[] = [{ domain: "github.com", groupName: "Dev", color: "blue" }];
  ...
  it("prefers exact rules over wildcard rules", () => {
    const exactTabs: TabInfo[] = [
      { id: 101, title: "Console", url: "https://console.aws.amazon.com/home" },
    ];
    const rules: DomainRule[] = [
...
describe("inferTargetGroup", () => {
  ...
  it("strips www. from hostname when checking rules", () => {
    ...
    expect(inferTargetGroup("https://www.youtube.com/watch", rules, {})).toEqual({
      name: "Media",
      color: "red",
    });
  });
  ...
  it("strips www. from hostname when checking affinity", () => {
    ...
    expect(inferTargetGroup("https://www.youtube.com/watch", [], affinity)).toEqual({
      name: "AffinityMedia",
    });
...
describe("buildPrompt", () => {
  ...
  it("handles tabs with special characters in title", () => {
    const special: TabInfo[] = [
      { id: 1, title: 'Tab "with" <special> & chars', url: "https://example.com" },
    ];
...
describe("parseResponse", () => {
  it("parses valid JSON array", () => {
    const raw =
      '[{"name":"Dev","color":"blue","tabIds":[1,2]},{"name":"Media","color":"red","tabIds":[3]}]';
  ...
  it("drops groups with no valid tabs", () => {
    const raw =
      '[{"name":"Empty","color":"blue","tabIds":[999]},{"name":"Dev","color":"red","tabIds":[1]}]';
    const result = parseResponse(raw, tabs);
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe("Dev");
  });
  ...
  it("handles empty tabIds array", () => {
    const raw =
      '[{"name":"Empty","color":"blue","tabIds":[]},{"name":"Dev","color":"red","tabIds":[1]}]';
    const result = parseResponse(raw, tabs);
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe("Dev");
  });
...
describe("matchTabsToExistingGroups", () => {
  ...
  it("puts unmatched tabs in remaining", () => {
    const testTabs: TabInfo[] = [{ id: 1, title: "Random Page", url: "https://example.com" }];
