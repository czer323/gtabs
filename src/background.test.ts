...
describe("hostnameFromUrl", () => {
  it("extracts hostname from a simple URL", () =>
    expect(hostnameFromUrl("https://github.com/repo")).toBe("github.com"));
  it("strips www prefix", () =>
    expect(hostnameFromUrl("https://www.example.com/path")).toBe("example.com"));
  it("handles subdomains without stripping non-www", () =>
  ...
  it("ignores port in returned hostname", () =>
    expect(hostnameFromUrl("http://localhost:3000/app")).toBe("localhost"));
});

describe("isImportantAppUrl", () => {
  it("identifies Gmail", () =>
    expect(isImportantAppUrl("https://mail.google.com/mail/u/0/#inbox")).toBe(true));
  it("identifies Slack", () =>
    expect(isImportantAppUrl("https://myteam.slack.com/messages")).toBe(true));
  it("identifies Notion", () =>
    expect(isImportantAppUrl("https://notion.so/my-workspace")).toBe(true));
...
describe("organize", () => {
  ...
  it("returns suggestions on success", async () => {
    mockFetchLLM(
      '[{"name":"Dev","color":"blue","tabIds":[1]},{"name":"Fun","color":"red","tabIds":[2]}]',
    );
    const result = await organize();
...
describe("applyGroups", () => {
  ...
  it("sets correct group titles and colors", async () => {
    ...
    expect(chrome.tabGroups.update).toHaveBeenCalledWith(
      201,
      expect.objectContaining({ title: "Media", color: "red" }),
    );
  });
...
describe("power tools", () => {
  ...
  it("consolidates tabs from other windows into the current one", async () => {
    ...
    vi.mocked(chrome.windows.getAll).mockResolvedValue([
      { id: 1, tabs: [{ id: 1, url: "https://here.com" }] },
      {
        id: 2,
        tabs: [
          { id: 21, url: "https://one.com" },
          { id: 22, url: "chrome://extensions" },
        ],
      },
  ...
  it("purges tabs older than the stale threshold", async () => {
    ...
    vi.mocked(chrome.tabs.query).mockResolvedValue([
      ...
      },
      {
        ...
        active: false,
        pinned: false,
        lastAccessed: now - 30 * 60 * 60 * 1000,
        groupId: -1,
      },
      {
        id: 3,
  ...
  it("deletes all groups in current window and returns cleared group count", async () => {
    await saveSuggestions([
      { name: "Old", color: "grey", tabs: [{ id: 1, title: "A", url: "https://a.com" }] },
    ]);
    vi.mocked(chrome.tabs.query).mockResolvedValue([
...
describe("event listeners", () => {
  ...
  it("checks auto-trigger on tab update even when silent auto add is off", async () => {
    ...
    await (chrome.tabs.onUpdated as any).callListeners(
      1,
      { status: "complete" },
      { url: "https://a.com", windowId: 1, groupId: -1 },
    );
  ...
  describe("system events", () => {
      ...
      expect(created).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            id: "gtabs-add-to-group-101",
            parentId: "gtabs-add-to-group",
            title: "Docs",
          }),
          expect.objectContaining({
            id: "gtabs-add-to-group-202",
            parentId: "gtabs-add-to-group",
            title: "Group 202",
          }),
        ]),
    ...
    it("ignores duplicate context menu create errors during rebuild", async () => {
      ...
      vi.mocked(chrome.contextMenus.create).mockImplementation(
        (_props: any, callback?: () => void) => {
          (chrome.runtime as any).lastError = {
            message: "Cannot create item with duplicate id gtabs-add-to-group",
          };
          callback?.();
    ...
    it("handles context menus", async () => {
      ...
      await (chrome.contextMenus.onClicked as any).callListeners({
        menuItemId: "gtabs-organize-ungrouped",
      });
      await (chrome.contextMenus.onClicked as any).callListeners({ menuItemId: "gtabs-undo" });
      await (chrome.contextMenus.onClicked as any).callListeners({
        menuItemId: "gtabs-duplicates",
      });
      expect(chrome.tabs.query).toHaveBeenCalled();
    });
  });
