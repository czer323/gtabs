...
export function _resetAutoCheckCooldown() {
  lastAutoCheckTime = 0;
}
...
export function isImportantAppUrl(url: string): boolean {
  ...
  return IMPORTANT_APP_PATTERNS.some(
...
async function getCurrentWindowId(): Promise<number> {
  ...
  try {
    ...
    /* service worker may not have a current window */
  }
  if (win?.id === undefined) {
    try {
...
export async function restoreSnapshot(snapshot: UndoSnapshot): Promise<void> {
  if (snapshot.ungrouped.length > 0) {
    try {
      ...
      /* stale tab IDs during restore — expected */
    }
  }
...
function rebuildContextMenus(): Promise<void> {
  ...
  contextMenuRebuildQueue = rebuild.catch((err) => {
    console.warn(
      "[gTabs] Failed to rebuild context menus:",
...
export async function organize(
  ...
  try {
    const [settings, affinity, domainRules, history, weightedAffinity, corrections, rejections] =
      await Promise.all([
        getSettings(),
        getAffinity(),
        getDomainRules(),
    ...
    if (settings.spendingCapUSD > 0) {
      ...
      if (costs.totalCost >= settings.spendingCapUSD) {
        return {
          error: `Spending cap of $${settings.spendingCapUSD.toFixed(2)} reached. Increase or disable in Settings.`,
        };
      }
    }
...
export async function applyGroups(suggestions: GroupSuggestion[]): Promise<void> {
  ...
  try {
    ...
    /* stale tab IDs during apply — expected */
  }
...
export async function sortCurrentGroupsByDomain(): Promise<number> {
  const tabs = (await chrome.tabs.query({ currentWindow: true })).filter(
    (tab) => tab.id !== undefined && isTabUrlAllowed(tab.url),
  );
...
chrome.runtime.onMessage.addListener((msg: MessageType, _sender, sendResponse) => {
  ...
  if (msg.type === "list-workspaces") {
    getWorkspaces()
      .then((ws) =>
        sendResponse({ type: "status", status: "done", workspaceNames: Object.keys(ws) }),
      )
      .catch((e) =>
        sendResponse({
          type: "status",
...
chrome.commands?.onCommand?.addListener((command: string) => {
  ...
  commandInFlightTimer = setTimeout(() => {
  ...
  if (alarm.name.startsWith(SNOOZE_ALARM_PREFIX)) {
    ...
    void (async () => {
      try {
        ...
        console.warn(
          "[gTabs] Failed to restore snoozed tab:",
          err instanceof Error ? err.message : err,
        );
      }
    })();
