...
describe("affinity", () => {
    ...
    await updateAffinity([
      {
        name: "Dev",
        color: "blue",
        tabs: [{ id: 1, title: "GH", url: "https://github.com/repo" }],
      },
    ]);
  ...
  it("newer group wins when called more times", async () => {
    await updateAffinity([
      { name: "Old", color: "blue", tabs: [{ id: 1, title: "", url: "https://github.com/x" }] },
    ]);
    // Call "New" twice so it has higher count
    await updateAffinity([
      { name: "New", color: "red", tabs: [{ id: 1, title: "", url: "https://github.com/y" }] },
    ]);
    await updateAffinity([
      { name: "New", color: "red", tabs: [{ id: 1, title: "", url: "https://github.com/y" }] },
    ]);
  ...
  it("preserves unrelated domains", async () => {
    await updateAffinity([
      { name: "A", color: "blue", tabs: [{ id: 1, title: "", url: "https://a.com" }] },
    ]);
    await updateAffinity([
      { name: "B", color: "red", tabs: [{ id: 2, title: "", url: "https://b.com" }] },
    ]);
  ...
  it("skips invalid URLs", async () => {
    await updateAffinity([
      { name: "X", color: "blue", tabs: [{ id: 1, title: "", url: "not-url" }] },
    ]);
    expect(await getAffinity()).toEqual({});
  });
});
    ...
    await updateWeightedAffinity([
      {
        name: "MyOrg",
        color: "blue",
        tabs: [{ id: 1, title: "", url: "https://github.com/myorg/repo" }],
      },
    ]);
    const w = await getWeightedAffinity();
