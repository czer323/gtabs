...
describe("complete - request format", () => {
  ...
  it("constructs URL from baseUrl correctly", async () => {
    mockOk("ok");
    await complete({ ...cfg, baseUrl: "https://custom.api.com/v2" }, [
      { role: "user", content: "hi" },
    ]);
    expect(fetch).toHaveBeenCalledWith(
      "https://custom.api.com/v2/chat/completions",
      expect.anything(),
    );
  });
  ...
  it("handles baseUrl with trailing slash", async () => {
    mockOk("ok");
    await complete({ ...cfg, baseUrl: "https://api.test.com/v1/" }, [
      { role: "user", content: "hi" },
    ]);
    expect(fetch).toHaveBeenCalledWith(
      "https://api.test.com/v1/chat/completions",
...
describe("complete - Anthropic API", () => {
  ...
  it("uses OpenAI format for non-Anthropic URLs", async () => {
    mockOk("ok");
    await complete({ ...cfg, baseUrl: "https://api.groq.com/openai/v1" }, [
      { role: "user", content: "hi" },
    ]);
    expect(fetch).toHaveBeenCalledWith(
...
describe("Chrome AI", () => {
  ...
  it("throws error if completeChromeAI called but missing", async () => {
    ...
    await expect(completeWithUsage(config, [{ role: "user", content: "hello" }])).rejects.toThrow(
