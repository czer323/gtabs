...
function renderProviderCards(selectedId: string) {
  ...
  for (const p of PROVIDERS) {
    ...
    card.addEventListener("click", () => {
      if (p.isBuiltIn && !chromeAIAvailable) {
        showChromeAISetup();
        return;
      }
      hideChromeAISetup();
...
async function save() {
  ...
  const settings: Settings = {
    ...
    staleTabThresholdHours:
      Number(inStaleTabThresholdHours.value) || DEFAULT_SETTINGS.staleTabThresholdHours,
...
async function load() {
  ...
  // Find provider
  const p =
    PROVIDERS.find((provider) => provider.id === s.provider) ||
...
importRulesFile.addEventListener("change", async () => {
  ...
  try {
    ...
    if (imported.length === 0) {
      alert("No valid rules found in CSV");
      return;
    }
...
const autoSaveElements = [
  ...
  inAutoPinApps,
  inSmartUngroup,
  inStaleTabThresholdHours,
  inSpendingCapUSD,
  inEnableCorrectionTracking,
  inEnableRejectionMemory,
  inEnableGroupDrift,
  inEnablePatternMining,
  inGroupDriftThreshold,
  inReorgSchedule,
  inReorgTime,
];
for (const el of autoSaveElements) {
  ...
  if (
    ...
    (el.type === "text" || el.type === "password" || el.type === "number" || el.type === "range")
  ) {
...
$<HTMLButtonElement>("tool-duplicates").addEventListener("click", async () => {
  ...
  for (const group of res.duplicates) {
    ...
    div.style.cssText =
      "background:rgba(242,139,130,0.05);border:1px solid rgba(242,139,130,0.12);border-radius:8px;padding:8px 10px;margin-bottom:6px;font-size:11px;color:#7a8099";
...
$<HTMLButtonElement>("tool-snooze").addEventListener("click", async () => {
  ...
  if (!activeTab?.id) {
    setToolStatus("No active tab to snooze", true);
    return;
  }
...
async function refreshToolWorkspaces() {
  ...
  for (const name of names) {
    ...
    const row = document.createElement("div");
    row.style.cssText =
      "display:flex;align-items:center;gap:6px;padding:5px 8px;border-radius:8px;margin-bottom:3px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);font-size:11px";
    row.innerHTML = `
...
$<HTMLButtonElement>("tool-workspace-save").addEventListener("click", async () => {
  ...
  if (!name) {
    setToolStatus("Enter a workspace name", true);
    return;
  }
