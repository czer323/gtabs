...
describe("E2E Integration: Message dispatch → Background → Storage", () => {
  ...
  it("organize then apply updates affinity", async () => {
    ...
    vi.mocked(fetch).mockResolvedValue(
      new Response(
        JSON.stringify({
          choices: [
            { message: { content: '[{"name":"Dev","tabIds":[1]},{"name":"Fun","tabIds":[2]}]' } },
          ],
  ...
  it("record-corrections boosts corrected group weight 3x", async () => {
    await sendMsg({
      ...
      corrections: {
        timestamp: Date.now(),
        corrections: [
          { domain: "newdomain12345.com", originalGroup: "Fun", correctedGroup: "CorrectedGroup" },
        ],
      },
    });
  ...
  it("record-rejections saves rejection data", async () => {
    const rejections = [{ timestamp: Date.now(), domain: "reddit.com", rejectedGroup: "Work" }];
  ...
  it("focus-group collapses all but active group", async () => {
    vi.mocked(chrome.tabs.query).mockImplementation(async (q: any) => {
      if (q?.active === true)
        return [{ id: 1, url: "https://a.com", groupId: 5, windowId: 1 }] as any;
      return [];
    });
    vi.mocked(chrome.tabGroups.query).mockResolvedValue([
      { id: 5, title: "Active" },
